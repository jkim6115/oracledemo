...
-- 테이블 모든 컬럼에 데이터를 삽입할 때는 컬럼명을 생략가능
INSERT INTO emp01(first_name, salary)
VALUES('Steven', 4000);
...
-- emp01 테이블 first_name컬럼의 값이 'candy'일 때
-- salary컬럼의 값을 8000으로 업데이트
UPDATE emp01
SET salary=8000
...
--emp01 테이블에서 first_name 컬럼의 값이 'candy'일 때 삭제하시오
DELETE FROM emp01
WHERE first_name = 'candy';
...
SELECT table_name frim user_tables
...
-- B세션
--A세션이 commit 되면 B세션은 행에 대한 lock상태가 해제되면서 오류메세지 출력
